...
fun sans(arr: JSONArray): ArrayList<ArrayList<String>> {
    var result: ArrayList<ArrayList<String>> = arrayListOf(arrayListOf("책이름", "대여", "책 번호", "http://vz.kro.kr/small.jpg"))
    for (i in 0 until arr.length()) {
            var b =  arr.getJSONObject(i)
            result.add(arrayListOf(b.getString("title").toString(),  b.getString("canRental").toString(), b.getString("number").toString(), b.getString("url").toString()))
        }
    return result
    //return "$result";
}
...
class MainActivity : AppCompatActivity() {
    ...
    var bookList = arrayListOf<Book>(
        Book("책이름", "대여", "책 번호", "http://vz.kro.kr/small.jpg"),
        Book("어린왕자", "가능", "ㄱ 5678", "http://vz.kro.kr/small.jpg")
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button.setOnClickListener() {
            CoroutineScope(IO).launch {
                ...
                val parse = Jsoup.connect("https://api.book.msub.kr/?book="+book.getText()+"&school="+school.getText()+"&local="+local.selectedItem)
                    .ignoreContentType(true).get().text()
                if (JSONObject(parse).getString("status") == "normal") {
                   ...
                   CoroutineScope(Main).launch  {
                    ...
                    CoroutineScope(Main).launch {
                        delay(50)
                        bookList.clear()
                        bookList.add(Book("책이름", "대여", "책 번호", "http://vz.kro.kr/small.jpg"))
